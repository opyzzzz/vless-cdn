...
VERSION="2.4-HTTP2-自动系统+IPv4/IPv6+Token固定隧道版"
...
[Unit]
Description=Suoha Cloudflare Tunnel (HTTP2 Token模式)
After=network.target
...
command_args="tunnel --protocol http2 --edge-ip-version $EDGE_IP_VERSION run --token $TOKEN"
command_background=true
EOF
